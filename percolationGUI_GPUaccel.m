function varargout = percolationGUI_GPUaccel(varargin)
% PERCOLATIONGUI_GPUACCEL MATLAB code for percolationGUI_GPUaccel.fig
%      PERCOLATIONGUI_GPUACCEL, by itself, creates a new PERCOLATIONGUI_GPUACCEL or raises the existing
%      singleton*.
%
%      H = PERCOLATIONGUI_GPUACCEL returns the handle to a new PERCOLATIONGUI_GPUACCEL or the handle to
%      the existing singleton*.
%
%      PERCOLATIONGUI_GPUACCEL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PERCOLATIONGUI_GPUACCEL.M with the given input arguments.
%
%      PERCOLATIONGUI_GPUACCEL('Property','Value',...) creates a new PERCOLATIONGUI_GPUACCEL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before percolationGUI_GPUaccel_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to percolationGUI_GPUaccel_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help percolationGUI_GPUaccel

% Last Modified by GUIDE v2.5 10-Jul-2016 10:35:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @percolationGUI_GPUaccel_OpeningFcn, ...
                   'gui_OutputFcn',  @percolationGUI_GPUaccel_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before percolationGUI_GPUaccel is made visible.
function percolationGUI_GPUaccel_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to percolationGUI_GPUaccel (see VARARGIN)


axis off
data=get(handles.b_start,'UserData');
data.gpu=gpuDevice(1);
...
% Choose default command line output for percolationGUI_GPUaccel
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes percolationGUI_GPUaccel wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = percolationGUI_GPUaccel_OutputFcn(hObject, eventdata, handles) 
...
percolation=gpuArray(data.percolation);wait(data.gpu);
lattice=gpuArray(data.baselattice>data.p);wait(data.gpu);
oldpercolation=gather(percolation);wait(data.gpu);
...
while (data.running == 1)
    ...
    %oldpercolation=gather(percolation);wait(data.gpu);
    %data.oldpercolation=gather(percolation);wait(data.gpu);
    
    str = cellstr(get(handles.pop_liqbeh,'String'));
    val = get(handles.pop_liqbeh,'Value');
    switch strtrim(str{val});
    case 'High Pressure'
        percolation = iteratePercolation_8N(percolation, lattice);wait(data.gpu);
    case 'Nearest Neighbor Only'
        percolation = iteratePercolation_4N(percolation, lattice);wait(data.gpu);
    case 'Low Gravity'
        percolation = iteratePercolation_LowGrav(percolation, lattice);wait(data.gpu);
    case 'High Gravity'
        percolation = iteratePercolation_HighGrav(percolation, lattice);wait(data.gpu);
    end
    ...
    if mod(n,data.stepSize)==0
        toc
        displayPercolation( gather(percolation), gather(lattice), data.color, oldpercolation );
        oldpercolation=gather(percolation);wait(data.gpu);
        tic
...
data.percolation=gather(percolation);
set(handles.b_start,'UserData',data);
